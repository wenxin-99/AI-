...
router.post("/stream", async (req: Request, res: Response) => {
  try {
    ...
    const automationPatterns = [
      ...
      // 注意："发技术帖"、"发一个详细的技术帖"等中间有修饰词的情况用 发.{0,10}帖 匹配
      /(帮我|帮忙|请).{0,20}(发.{0,10}帖|发贴|发布帖子|发布内容|发布.{0,6}帖)/i,
      /(帮我|帮忙|请).{0,20}(运营|管理|运行).{0,20}(论坛|论谈|网站|社区|板块)/i,
      /(帮我|帮忙|请).{0,30}(回帖|回贴|回复帖子|评论)/i,
      /(自动|批量).{0,20}(发.{0,10}帖|发贴|回帖|回贴|发布|评论)/i,
      /(论坛|论谈|网站|社区).{0,20}(发.{0,10}帖|发贴|运营|回复|互动)/i,
      /(登录|登陆|登入).{0,20}(论坛|论谈|网站|社区).{0,20}(发.{0,10}帖|发贴|回复|运营)/i,
      /发帖运营|发贴运营/i,
      /论坛运营|论谈运营/i,
      /网站运营/i,
      // 包含账号密码信息的发帖请求（允许"发技术帖"等中间有修饰词）
      /(账号|帐号|用户名|账户).{0,30}(密码|密吗|pwd).{0,80}(发.{0,10}帖|发贴|运营|回复|发布)/i,
      /(发.{0,10}帖|发贴|运营|回复|发布).{0,80}(账号|帐号|用户名|账户).{0,30}(密码|密吗|pwd)/i,
      // 模糊表达："帮我去那个论坛发点东西"
      /(帮我|帮忙).{0,30}(去|到|上).{0,20}(论坛|网站|社区).{0,20}(发|写|弄)/i,
      // 关键组合检测：同时包含 论坛/网址 + 账号/密码 + 发/帖/运营
      /(?=.*(论坛|网站|社区|\.com|\.cn|\.net|\.org))(?=.*(账号|帐号|用户名|密码|pwd))(?=.*(发.{0,10}帖|发贴|运营|回复|发布|帖子))/i,
      // 包含网址 + 账号密码 + 帮忙/请 的组合
      /(?=.*(\.com|\.cn|\.net|\.org))(?=.*(账号|帐号|用户名))(?=.*(密码|pwd))(?=.*(帮|请|发|运营))/i,
      // 英文模式
