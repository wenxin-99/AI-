...
export const trafficService = {
  ...
  getUserTraffic: async (userId: number): Promise<UserTrafficStats> => {
    const response = await api.get<any, ApiResponse<UserTrafficStats>>(
      `/api/v1/traffic/user/${userId}`
    );
  ...
  getInboundTraffic: async (inbound: string): Promise<InboundTrafficStats> => {
    const response = await api.get<any, ApiResponse<InboundTrafficStats>>(
      `/api/v1/traffic/inbound/${inbound}`
    );
    return response.data;
  },
  ...
  getSystemTraffic: async (): Promise<SystemTrafficStats> => {
    const response = await api.get<any, ApiResponse<SystemTrafficStats>>(
      "/api/v1/traffic/system"
    );
    return response.data;
  },
  ...
  getTrafficTrend: async (days: number = 7): Promise<TrafficTrend[]> => {
    const response = await api.get<any, ApiResponse<TrafficTrend[]>>(
      "/api/v1/traffic/trend",
      {
        params: { days },
      }
    );
